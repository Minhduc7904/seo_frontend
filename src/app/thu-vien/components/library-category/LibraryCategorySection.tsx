...
import { BookOpenText, Check, CircleHelp } from "lucide-react";
...
export default function LibraryCategorySection() {
    return (
        <section className="rounded-[1.6rem] bg-white p-6 md:p-7">
            <h2 className="text-center text-2xl font-extrabold leading-tight text-slate-900">Chọn thư viện</h2>

            <div className="mt-6 grid gap-5 md:grid-cols-2">
                {CATEGORIES.map(({ href, title, description, Icon, highlights, featured }) => {
                    const baseClasses =
                    ...
                    const descriptionClasses = featured ? "text-blue-100" : "text-slate-600";
                    const listTextClasses = featured ? "text-blue-50" : "text-slate-700";
                    ...
                    return (
                        <Link key={href} href={href} className={`${baseClasses} ${themeClasses}`}>
                            <div className="flex items-center gap-3">
                                <span className={`inline-flex h-11 w-11 items-center justify-center rounded-xl ${iconClasses}`}>
                                    <Icon className="h-5 w-5" />
                                </span>
                                <p className="text-lg font-bold">{title}</p>
                            </div>
                            <p className={`mt-3 text-sm leading-6 ${descriptionClasses}`}>{description}</p>
                            <ul className={`mt-5 space-y-2 text-sm leading-6 ${listTextClasses}`}>
                                {highlights.map((item) => (
                                    <li key={item} className="flex items-start gap-2">
                                        <Check className={`mt-0.5 h-4 w-4 flex-shrink-0 ${bulletClasses}`} />
                                        <span>{item}</span>
                                    </li>
                                ))}
                            </ul>
