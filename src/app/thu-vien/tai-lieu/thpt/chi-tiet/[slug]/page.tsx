...
export async function generateMetadata({ params }: DocumentDetailPageProps): Promise<Metadata> {
    ...
    try {
        ...
        return buildDocumentDetailMetadata(payload.data, `/thu-vien/tai-lieu/thpt/chi-tiet/${slug}`);
    } catch {
        return {};
    }
}

export default async function ThptDocumentDetailPage({ params }: DocumentDetailPageProps) {
    const { slug } = await params;
